...
if __name__ == '__main__':
    ...
    image = cv2.imread('bear.jpg', 1)
    #outimg,edges,seeds=auto_region_growing(image)
    # cv2.imshow('Region Growing', outimg)
    # cv2.waitKey()
    #imgInput = sitk.ReadImage('nemo.jpg')
    image=cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
    ...
    for d in clicks:
        print(image[d[0],d[1]])
    e=image[c[0], c[1]]
    #seg_con = sitk.ConfidenceConnected(imgInput, seedList=clicks,numberOfIterations=1,multiplier=2.5 ,initialNeighborhoodRadius=2,replaceValue=1)
    seg_con = sitk.ConnectedThreshold(imgInput, seedList=clicks,lower=int(e-50), upper=int(e+50))
    img_arr = sitk.GetArrayFromImage(seg_con)
    sitk_show(seg_con)
    target = np.array(255 * img_arr, dtype='uint8')
    cv2.imshow('qwerty',target)
    #cv2.imshow('edges',seg_con)
    cv2.waitKey()
